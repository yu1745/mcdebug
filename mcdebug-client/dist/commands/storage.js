...
export function registerStorageCommands(parent, getApi) {
    ...
    storage
        ...
        const r = await api.storage.insert(await parseTarget(opts, 'storage insert'), opts.handle ?? defaultHandle(resource), resource, amount, { side: normalizeSide(opts.side), simulate: opts.simulate });
        outputOneLineJson(r);
        await api.close();
    });
    storage
        ...
        const resource = await parseResource(opts);
        const amount = parseAmount(opts.amount);
        const r = await api.storage.extract(await parseTarget(opts, 'storage extract'), opts.handle ?? defaultHandle(resource), resource, amount, { side: normalizeSide(opts.side), simulate: opts.simulate });
        outputOneLineJson(r);
        await api.close();
    });
    storage
        ...
        .action(async (opts) => {
        const api = getApi();
        const r = await api.storage.transfer(await parseEndpointTarget(opts.fromTarget, opts.from, opts.dim, 'storage transfer --from'), await parseEndpointTarget(opts.toTarget, opts.to, opts.dim, 'storage transfer --to'), await parseResource(opts), parseAmount(opts.amount), { fromSide: normalizeSide(opts.fromSide), toSide: normalizeSide(opts.toSide), simulate: opts.simulate });
        outputOneLineJson(r);
        await api.close();
    });
}
async function parseTarget(opts, what) {
    if (opts.target)
        return (await parseJsonArg(opts.target));
    return { kind: 'block', pos: requirePos(opts, what), dim: opts.dim };
}
async function parseEndpointTarget(json, posSpec, dim, what) {
    if (json)
        return (await parseJsonArg(json));
    if (!posSpec)
        throw new Error(`${what} requires --from/--to or --from-target/--to-target`);
    return { kind: 'block', pos: parseTriplet(posSpec, what), dim };
}
async function parseResource(opts) {
    if (opts.resource)
        return (await parseJsonArg(opts.resource));
    const nbt = opts.nbt ? (await parseJsonArg(opts.nbt)) : undefined;
    if (opts.item)
        return { kind: 'item', item: opts.item, nbt };
    if (opts.fluid)
        return { kind: 'fluid', fluid: opts.fluid, nbt };
    if (opts.energy)
        return { kind: 'energy' };
    throw new Error('resource required: use --resource, --item, --fluid, or --energy');
}
function defaultHandle(resource) {
