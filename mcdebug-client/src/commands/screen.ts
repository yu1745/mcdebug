...
import type { ItemStackJson, Side } from '../types.js';
import { outputJson, outputOneLineJson, parseJsonArg, requirePos } from './util.js';

type SlotActionType = 'pickup' | 'quick_move' | 'swap' | 'clone' | 'throw' | 'quick_craft' | 'pickup_all';

export function registerScreenCommands(parent: Command, getApi: () => DebugApi): void {
  ...
  screen
    ...
    .action(async (opts: { x: string; y: string; z: string; dim?: string; side?: string }) => {
      const api = getApi();
  ...
  screen
    ...
    .action(async (opts: { screenId: string }) => {
      const api = getApi();
  ...
  screen
    ...
    .action(async (opts: { screenId: string; slot: string; item: string; count: string; nbt?: string }) => {
      const api = getApi();
  ...
  screen
    ...
    .action(async (opts: { screenId: string; slot: string; button: string; actionType: SlotActionType }) => {
      const api = getApi();
  ...
  screen
    ...
    .requiredOption('--screen-id <id>', 'screen id')
    .requiredOption('--slot <n>', 'handler slot index')
    .action(async (opts: { screenId: string; slot: string }) => {
      const api = getApi();
...
async function parseStack(opts: { item: string; count: string; nbt?: string }): Promise<ItemStackJson> {
  ...
  return {
    ...
    count,
    nbt: opts.nbt ? ((await parseJsonArg(opts.nbt)) as ItemStackJson['nbt']) : undefined,
  };
}

function parseSlot(value: string): number {
  const slot = Number(value);
  if (!Number.isInteger(slot)) throw new Error('slot/button must be an integer');
  return slot;
}
