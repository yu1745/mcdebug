...
import type { ItemStackJson, JsonNbt } from '../types.js';
...
export function registerEntityCommands(cmd: Command, getApi: () => DebugApi): void {
  ...
  entity
    ...
    .action(async (opts: { type: string; pos: string; dim?: string; yaw?: string; pitch?: string; nbt?: string; stack?: string; includeNbt?: boolean }) => {
      const api = getApi();
      const r = await api.entity.spawn(opts.type, parseTriplet(opts.pos, '--pos'), {
        ...
        nbt: opts.nbt ? ((await parseJsonArg(opts.nbt)) as JsonNbt) : undefined,
        stack: opts.stack ? ((await parseJsonArg(opts.stack)) as ItemStackJson) : undefined,
  ...
  entity
    ...
    .option('--dim <id>', 'dimension id')
    .action(async (opts: { uuid: string; dim?: string }) => {
      const api = getApi();
  ...
  entity
    ...
    .action(async (opts: { uuid: string; nbt: string; dim?: string; replace?: boolean }) => {
      const api = getApi();
      const r = await api.entity.setNbt(opts.uuid, (await parseJsonArg(opts.nbt)) as JsonNbt, {
        dim: opts.dim,
  ...
  entity
    ...
    .option('--yaw <n>', 'yaw degrees')
    .option('--pitch <n>', 'pitch degrees')
    .action(async (opts: { uuid: string; pos: string; dim?: string; toDim?: string; yaw?: string; pitch?: string }) => {
      const api = getApi();
  ...
  entity
    ...
    .action(async (opts: { from: string; to: string; item?: string; dim?: string; includeNbt?: boolean }) => {
      const api = getApi();
      const r = await api.entity.listItems(
        { from: parseTriplet(opts.from, '--from'), to: parseTriplet(opts.to, '--to') },
        { item: opts.item, dim: opts.dim, includeNbt: !!opts.includeNbt },
      );
      outputJson(r);
      await api.close();
    });
  ...
  entity
    ...
    .action(async (opts: { from: string; to: string; item?: string; dim?: string; keep?: boolean }) => {
      const api = getApi();
      const r = await api.entity.collectItems(
        { from: parseTriplet(opts.from, '--from'), to: parseTriplet(opts.to, '--to') },
        { item: opts.item, dim: opts.dim, remove: !opts.keep },
      );
      outputJson(r);
      await api.close();
    });
