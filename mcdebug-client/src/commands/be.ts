...
export function registerBeCommands(cmd: Command, getApi: () => DebugApi): void {
  cmd
    .command('get-nbt')
  ...
  cmd
    ...
    .option('--dim <id>', 'dimension id')
    .action(
      async (opts: { x: string; y: string; z: string; nbt: string; dim?: string }) => {
        const api = getApi();
        const parsed = await parseJsonArg(opts.nbt);
        const r = await api.be.setNbt(requirePos(opts, 'be set-nbt'), parsed as never, opts.dim);
        outputOneLineJson(r);
        await api.close();
      },
    );
  ...
  cmd
    ...
    .option('--dim <id>', 'dimension id')
    .action(
      async (opts: { x: string; y: string; z: string; path: string; dim?: string }) => {
        const api = getApi();
  ...
  cmd
    ...
    .action(
      async (opts: {
        ...
        const r = await api.be.setField(
          requirePos(opts, 'be set-field'),
          opts.path,
          parsed as never,
          opts.dim,
        );
