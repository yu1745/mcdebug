...
export function registerInvCommands(parent: Command, getApi: () => DebugApi): void {
  ...
  inv
    ...
    .action(
      async (opts: { x: string; y: string; z: string; slot: string; dim?: string }) => {
        const api = getApi();
  ...
  inv
    ...
    .action(
      async (opts: {
        ...
        const nbtJson = opts.nbt ? ((await parseJsonArg(opts.nbt)) as never) : undefined;
        const r = await api.inv.setSlot(
          requirePos(opts, 'inv set'),
          slot,
          opts.item === '' ? null : opts.item,
          count,
          nbtJson,
          opts.dim,
        );
