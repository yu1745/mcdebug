...
export function registerWorldCommands(cmd: Command, getApi: () => DebugApi): void {
  cmd
    ...
    .action(
      async (opts: { block: string; x: string; y: string; z: string; state?: string[]; dim?: string; flags?: string }) => {
        const api = getApi();
        ...
        const r = await api.world.setBlock(
          pos,
          opts.block,
          stateProps ? { name: opts.block, props: stateProps } : undefined,
          { dim: opts.dim, flags },
        );
        outputOneLineJson(r);
        await api.close();
  ...
  cmd
    ...
    .action(async (opts: { x: string; y: string; z: string; dim?: string }) => {
      const api = getApi();
  ...
  cmd
    ...
    .option('--dim <id>', 'dimension id')
    .action(async (opts: { x: string; y: string; z: string; nbt?: boolean; dim?: string }) => {
      const api = getApi();
  ...
  cmd
    ...
    .option('--dim <id>', 'dimension id')
    .action(
      async (opts: { from: string; to: string; block: string; count?: boolean; dim?: string }) => {
        const api = getApi();
        const r = await api.scan.findBlocks(
          { from: parseTriplet(opts.from, '--from'), to: parseTriplet(opts.to, '--to') },
          opts.block,
          { count: opts.count, dim: opts.dim },
        );
        outputJson(r);
        await api.close();
      },
  ...
  cmd
    ...
    .requiredOption('--to <x,y,z>', 'box to-corner (inclusive), 3 ints')
    .option('--dim <id>', 'dimension id')
    .action(async (opts: { from: string; to: string; dim?: string }) => {
      const api = getApi();
  ...
  cmd
    ...
    .option('--dim <id>', 'dimension id')
    .action(async (opts: { cx: string; cz: string; dim?: string }) => {
      const api = getApi();
      const cx = Number(opts.cx);
      const cz = Number(opts.cz);
      if (!Number.isInteger(cx) || !Number.isInteger(cz)) throw new Error('--cx and --cz must be integers');
