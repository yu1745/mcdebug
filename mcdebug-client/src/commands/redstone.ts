...
export function registerRedstoneCommands(parent: Command, getApi: () => DebugApi): void {
  ...
  redstone
    ...
    .action(async (opts: { x: string; y: string; z: string; side?: Direction; dim?: string }) => {
      const api = getApi();
  ...
  redstone
    ...
    .option('--dim <id>', 'dimension id')
    .action(async (opts: { x: string; y: string; z: string; powered: string; dim?: string }) => {
      const api = getApi();
  ...
  redstone
    ...
    .option('--dim <id>', 'dimension id')
    .action(async (opts: { x: string; y: string; z: string; ticks: string; dim?: string }) => {
      const api = getApi();
